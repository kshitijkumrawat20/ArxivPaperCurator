...
class OpenSearchClient: 
    ...
    def index_paper(self, paper_data: Dict[str, Any]) -> bool: 
        ...
        : param paper_data: Dictionary containing paper fields (arxiv_id, title, authors, abstract, etc.)
        ...
        try: 
            ...
            # convert authors list to string if need 
            ...
            response = self.client.index(
                ...
                body = paper_data,
                refresh = True  # make the document searchable immediately
            )
            ...
            if response.get("result") in ["created", "updated"]:
                ...
                logger.error(f"Failed to index paper {paper_data['arxiv_id']}. Response: {response}")
                return False
            

        except Exception as e:
            logger.error(f"Error indexing paper {paper_data.get('arxiv_id')}: {str(e)}")
            return False
    ...
    def search_papers(
        ...
        try:
            ...
            logger.info(f"Search query '{query}' returned {results['total']} hits.")
            return results
        
        except NotFoundError: 
            logger.error(f"Index {self.index_name} not found for search operation.")
