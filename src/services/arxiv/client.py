...
from functools import cached_property # what it does is that it caches the result of a property method so that it is only computed once and then stored for future access. This can improve performance if the property is expensive to compute and is accessed multiple times.
from typing import Dict, List , Optional 
from pathlib import Path 
from urllib.parse import urlencode, quote 
import httpx 
from src.config import ArxivSettings
from src.exception import ArxivAPIException, ArxivAPIRateLimitError, ArxivAPITimeoutError, ArxivParseError,PDFDownloadTimeoutError, PDFDownloadException
from src.schema.arxiv.paper import ArxivPaper
...
class ArxivClient:
    ...
    async def fetch_paperts(
        ...
        if from_date or to_date: 
            # convert dates to arxiv format YYYYMMDDHHMM - use 0000 for start of the day and 2359 for end of the day
            date_from = f"{from_date}0000" if from_date else "*"
            date_to = f"{to_date}2359" if to_date else "*"
            # use the correct arxiv API syntax with + symbols 
        ...
        safe = ":+[]" # dont encode:, +, [, ] chacters needed for arxiv queries 
        url = f"{self.base_url}?{urlencode(params,quote_via=quote, safe=safe)}"

        try: 
            logger.info(f" Fetching {max_results} {self.search_category} papers from arXiv API starting at index {start}...")

            # add rate limiting delay between all requests 
            if self._last_request_time is not None:
                time_since_last_request = time.time() - self._last_request_time
                if time_since_last_request < self.rate_limit_delay:
                    sleep_time = self.rate_limit_delay - time_since_last_request
                    logger.info(f"Rate limiting in effect. Sleeping for {sleep_time:.2f} seconds before making the next request.")
                    await asyncio.sleep(sleep_time)
            
            async with httpx.AsyncClient(timeout = self.timeout_seconds) as client:
                response = await client.get(url)
                self._last_request_time = time.time()  # Update the last request time after the request is made
    ...
    async def fetch_paper_by_id(self, arxiv_id: str) -> Optional[ArxivPaper]:
        ...
        """
        # cleaning the arxiv_id to remove version if present
        clean_id = arxiv_id.split("v")[0] if "v" in arxiv_id else arxiv_id
        params = { 
            "id_list": clean_id, "max_results": 1
        }
    ...
    def _parse_response(self, xml_data: str) -> List[ArxivPaper]:
        ...
        """
        try:
            root = ET.fromstring(xml_data) # parse the XML data into an ElementTree object example of element tree: <feed xmlns="http://www.w3.org/2005/Atom"> <entry> ... </entry> </feed>
            entries = root.findall("atom:entry", self.namespaces)# find all entry elements in the XML using the atom namespace
            papers = []
