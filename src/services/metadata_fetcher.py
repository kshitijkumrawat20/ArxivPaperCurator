...
from src.exception import MetadataFetchingException, PipelineException
from src.respositories.paper import PaperRepository
from src.schema.arxiv.paper import ArxivPaper, PaperCreate
from src.schema.pdf_parser.models import ArxivMetadata, ParsedPaper, PdfContent
...
class MetadataFetcher:
    ...
    async def fetch_and_process_papers(
        ...
        try:
            ...
            # # Step 4: Index in OpenSearch if requested
            # if index_to_opensearch and self.opensearch_client:
            #     logger.info("Step 4: Indexing papers in OpenSearch...")
    ...
    async def _process_pdfs_batch(self, papers: List[ArxivPaper]) -> Dict[str, Any]:
        ...
        for paper, result in zip(papers, pipeline_results):
            if isinstance(result, Exception):
                ...
                # check if resuylt is tuple before unpacking 
                # handle AirflowTaskTerminated and other non-tuple results 
                if isinstance(result, tuple) and len(result) == 2:
                    download_success, parsed_paper = result
                else:
                    # result is not tuple could be airflowTaskTermainal or other error 
                    error_msg = f"Pipeline error for {paper.arxiv_id}: Unexpected result type {type(result)}"
                    logger.error(error_msg)
                    results["errors"].append(error_msg)
                    continue
