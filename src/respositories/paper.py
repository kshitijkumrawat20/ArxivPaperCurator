...
from src.schema.arxiv.paper import PaperCreate
...
class PaperRepository:
    ...
    def get_by_arxiv_id(self, arxiv_id: str) -> Optional[Paper]:
        ...
        return self.session.scalar(stmt) # what scaler does is it returns the first result of the query or None if no result is found. It is used here to fetch a single Paper object based on the provided arxiv_id.
    ...
    def get_processed_papers(self, limit: int = 100, offset: int = 0) -> List[Paper]:
        stmt = (
            select(Paper).where(Paper.pdf_processed ==True).order_by(Paper.published_date.desc()).limit(limit).offset(offset)
        )
